...
namespace eosio { namespace vm {
   class binary_parser {
    public:
      binary_parser(maybe_allocator& alloc) : _allocator(alloc) {}
      ...
      outcome::result<result_void> parse_init_expr(wasm_code_ptr& code, init_expr& ie) {
         ie.opcode = *code++;
         switch (ie.opcode) {
      ...
      void parse_function_body_code(wasm_code_ptr& code, size_t bounds, guarded_vector<opcode>& fb) {
         ...
         while (code.offset() < bounds) {
            ...
            switch (*code++) {
               ...
               case opcodes::f32_const:
                  fb[op_index++] = f32_const_t{ *(uint32_t*)code.raw() };
                  code += 4;
                  break;
               case opcodes::f64_const:
                  fb[op_index++] = f64_const_t{ *(uint64_t*)code.raw() };
                  code += 8;
                  break;
