...
namespace eosio { namespace vm {
   ...
   template <typename T, typename Owner, typename Allocator, typename ErrHandler>
   class managed_vector {
    public:
      constexpr managed_vector(Owner& owner, Allocator& allocator, ErrHandler&& error_handler, size_t size = 0)
          : _size(size), _owner(&owner), _allocator(&allocator) {
         if (const auto& ptr = allocator.template alloc<T>(_size))
            _data = ptr.value();
         else
            error_handler(*owner);
      }
      ...
      constexpr inline void resize(size_t size) {
         if (size > _size) {
            if (const auto& ptr = _allocator->template alloc<T>(size)) {
               if (_size == 0)
                  _data = ptr.value();
            } else {
            }
      ...
      constexpr inline void   set(T* data, size_t size) {
         _size  = size;
         _data  = data;
         _index = size - 1;
      }
   ...
   std::string vector_to_string(T&& vec) {
      ...
      for (int i = 0; i < vec.size(); i++) str[i] = vec[i];
      return str;
   }
}} // namespace eosio::vm
